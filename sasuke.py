...
bot = telebot.TeleBot('7727622197:AAFq7IK_go2QK_Zgdfyp6gOe9cUrNPLpp0c')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, \n✨Premium Attack 𝐒𝐓𝐀𝐑𝐓ING ACTION .\n\n📡 𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n🔌 𝐏𝐨𝐫𝐭: {port}\n⏱️ 𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown. Please Wait 150 sec Before Running The /attack1 Command Again buy premium instant attack with zero ."
                bot.reply_to(message, response)
                return
